...
This is the **zukauskenOS Family Health OS fork** of obsidian-second-brain, a Claude Code skill that turns an Obsidian vault into a living AI-first second brain. The fork ships 18 slash commands across 3 layers (vault management, thinking tools, meta) - the upstream research toolkit commands, scheduled agents, and daily/kanban workflow commands are deliberately removed (see `DELTAS.md`).
...
- `commands/` - 18 slash command definitions, one `.md` per command. **This is the platform-neutral source.** Adapters compile it for each platform.
