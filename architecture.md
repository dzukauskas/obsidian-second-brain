...
- **18 commands** (this fork), grouped by `category:` frontmatter: vault 4, thinking 8, research 1, meta 5. All 18 are cross-platform. (Upstream ships 44; the Google Calendar family, research toolkit commands, and daily/kanban workflow commands are removed here - see `DELTAS.md`.)
- The research toolkit's Python layer remains in `scripts/research/` (neutralized: flat `research/` staging, per-day logs, no folder creation) but has no command surface in this fork.
- An opt-in background agent (triple-gated, ships disabled).
- MIT licensed.
...
| Path | Role |
|---|---|
| `commands/` | 18 slash-command definitions, one `.md` each. The platform-neutral source and the product surface. |
...
|-- commands/            # 18 command .md files (the source)
...
graph TD
  ...
  CMD["commands/<br/>18 .md, platform-neutral source"]
